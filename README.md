...
| `DISABLE_X509_STRICT_VALIDATION` | Set to `true` to disable X509_STRICT validation for Kubernetes API calls. Workaround for compatibility with old root CA certificates (created using k8s <= 1.16) that are not compliant with OpenSSL's VERIFY_X509_STRICT used by default in Python 3.13+.                                                                         | false    | -                                         | boolean |
